...
Each script below fetches its own external dependency before building.
...
# ./build_torch.sh      # Optional: builds embedded Tor support
```

> **Note:** This step takes a long time. Build artifacts are cached under `cw_monero/macos/`.
...
Open `macos/Runner.xcworkspace` in Xcode to archive and sign the application.
